<template>
  <IxSpace vertical>
    <IxProTagSelect
      ...
      :confirmBeforeSelect="forceConfirmSelect ? 'force' : true"
      selectConfirmHeader="确定"
      style="width: 336px"
    >
      <template #removeConfirmTitle="{ label }"> 确定删除“{{ label }}”标签吗？ </template>
      <template #removeConfirmContent> 操作说明 </template>
      <template #selectConfirmContent="{ label }"> 确定选择“{{ label }}”标签吗？ </template>
    </IxProTagSelect>
    <IxSpace>
      <span>force: </span>
      <IxSwitch v-model:checked="forceConfirmSelect" />
    </IxSpace>
  </IxSpace>
</template>
...
const forceConfirmSelect = ref(false)
