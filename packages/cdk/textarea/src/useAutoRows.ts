...
import { type BoxSizingData, getBoxSizingData, off, on, rAF } from '@idux/cdk/utils'
...
export function useAutoRows(
  ...
  let sizingStyle: BoxSizingData | undefined
  ...
  function getSizingData(): BoxSizingData {
    return sizingStyle ?? (sizingStyle = getBoxSizingData(textareaRef.value!))
  }
  ...
  function getTextareaScrollHeight() {
    ...
    return measureTextarea(textareaRef.value!, textarea => {
      ...
      return getHeightByScrollHeight(textarea.scrollHeight, sizingData)
    })
  }

  function getTextareaPlaceholderHeight() {
    if (!textareaRef.value || !!cachedPlaceholderHeight) {
      return 0
    }
    ...
    if (!cachedPlaceholderHeight) {
      ...
      return measureTextarea(textareaRef.value!, textarea => {
        ...
        cachedPlaceholderHeight = getHeightByScrollHeight(textarea.scrollHeight, sizingData)
      })
    }
  ...
  function resizeToFitContent(force = false) {
    ...
    const scrollHeight = getTextareaScrollHeight()
    const height = Math.max(scrollHeight, getTextareaPlaceholderHeight()!)
  ...
  function setMinHeight(): void {
    ...
    const minHeight = contentHeight ? `${getHeightByContentHeight(contentHeight, getSizingData())}px` : ''
  ...
  function setMaxHeight(): void {
    ...
    const maxHeight = contentHeight ? `${getHeightByContentHeight(contentHeight, getSizingData())}px` : ''
  ...
  onMounted(() => {
    ...
    stopWatch = () => watchStopHandlers.forEach(stop => stop())
