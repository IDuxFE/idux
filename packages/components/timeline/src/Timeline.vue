...
export default defineComponent({
  ...
  render() {
    ...
    const penddingItem = pendingNode ? h(IxTimelineItem, {}, {
      default: () => pendingNode,
      dot: () => pendingDotNode
    }) : null;
    ...
    const items = nonNullItems.map((item, index) => {
      let pendingCls = ''
      let position = getItemPosition(item as VNode, index, this.$props.mode);
      let lastCls = index === itemsLength - 1 ? `${itemPrefixCls}--last` : '';

      itemPositionArr.push(position);

      if (pendingNode) {
        pendingCls = (this.$props.reverse && index === 0) || (!this.$props.reverse && index === itemsLength - 2) ? 
      `${itemPrefixCls}--pending` : '';
      }
