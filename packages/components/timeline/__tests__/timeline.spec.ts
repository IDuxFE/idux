...
describe('Timeline.vue', () => {
    ...
    test('pendingDot work', async () => {
        ...
        expect(wrapper.find('.ix-timeline-item--last').find('.ix-timeline-item__dot').find('.ix-icon').classes()).toContain('ix-icon-loading');
        expect(wrapper.html()).toMatchSnapshot()

        await wrapper.setProps({ pendingDot: pendingDotText });
        expect(wrapper.find('.ix-timeline-item--last').find('.ix-timeline-item__dot').text()).toBe(pendingDotText);
        expect(wrapper.html()).toMatchSnapshot()
      })
    ...
    test('pendingDot slot work', async () => {
        ...
        expect(wrapper.find('.ix-timeline-item--last').find('.ix-timeline-item__dot').findAll('.ix-icon').length).toBe(0);
        expect(wrapper.find('.ix-timeline-item--last').find('.ix-timeline-item__dot').text()).toBe(pendingDotSlotText);
        expect(wrapper.html()).toMatchSnapshot()
