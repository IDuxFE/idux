...
import type { RowGutter, RowGutterRecord } from './types'
...
export const isRecordGutter = (gutter: unknown): gutter is RowGutterRecord => isObject(gutter) && !isArray(gutter)

// gutter is [{xs: 10}, {xs: 12}]
export const isRecordArrGutter = (gutter: unknown): gutter is [RowGutterRecord, RowGutterRecord] =>
  isArray(gutter) && (isRecordGutter(gutter[0]) || isRecordGutter(gutter[1]))

export const haveBreakpointGutter = (gutter: RowGutter): boolean => isRecordGutter(gutter) || isRecordArrGutter(gutter)

// gutter: RowGutter => [0, 0]
export const normalizeGutter = (propGutter: RowGutter, screens: Ref<ScreenMatch>): GutterType => {
