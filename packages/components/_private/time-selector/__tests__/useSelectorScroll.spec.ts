...
import { useSelectorScroll } from '../src/composables/useSelectorScroll'
...
describe('useSelectorScroll', () => {
  ...
  const initTestContext = (selectedValue?: number) => {
    ...
    const props: TimeSelectorColumnProps = reactive({
      selectedValue: selectedValue ?? 1,
      options,
      onChange,
    })
    const scrollAPI = useSelectorScroll(
      props,
  ...
  test('adjustSelector', async () => {
    const { wrapper, adjustSelector } = initTestContext()
